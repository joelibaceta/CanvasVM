...
const PI_PNG: &[u8] = include_bytes!("../../../tools/fixtures/samples/PI.png");
const AZ_PNG: &[u8] = include_bytes!("../../../tools/fixtures/samples/AZ.png");
...
fn test_hello_world_execution() {
    ...
    canvas
        .paint(&rgba_data, width as usize, height as usize)
        .unwrap();
    ...
    assert!(
        executed_steps > 0,
        "HelloWorld should execute at least one step"
    );
...
fn test_pi_loads() {
    let mut canvas = Canvas::new();
    let (rgba_data, width, height) = decode_png(PI_PNG);

    let result = canvas.paint(&rgba_data, width as usize, height as usize);
    assert!(result.is_ok(), "PI.png should load successfully");
}
...
fn test_pi_execution() {
    ...
    let output = canvas.ink_string().unwrap();
    console::log_1(&format!("PI output: {} ({} steps)", output, executed_steps).into());
    assert!(executed_steps > 0, "PI should execute");
    assert!(
        output.contains("3"),
        "PI should output digits starting with 3"
    );
}
...
fn test_all_examples_compile_to_bytecode() {
    ...
    for (name, png_bytes) in &examples {
        ...
        canvas
            .paint(&rgba_data, width as usize, height as usize)
            .unwrap_or_else(|_| panic!("{} should load", name));

        let bytecode = canvas
            .compile_to_bytecode()
            .unwrap_or_else(|_| panic!("{} should compile", name));
...
fn test_all_examples_reset_workflow() {
    ...
    for (name, png_bytes) in &examples {
        ...
        assert!(
            stroke_result.is_ok() || stroke_result.is_err(),
            "{} should be executable after reset",
            name
        );
...
fn test_hello_world_full_web_workflow() {
    ...
    // 3. snapshot() inicial
    let _snapshot = canvas.snapshot().unwrap();
    ...
    // Test exitsó si llegamos aquí sin panic
}
